...
class Downloader(threading.Thread):
    ...
    def update_database(self, result=1):
        ...
        try:
            ...
            cursor.execute('update console_apps set downloaded = ? where url = ?',
                    (result, self.url,))
            connection.commit()
...
def get_undownloaded_url(database_filepath):
    ...
    try:
        ...
        cursor = connection.cursor()
        matching_key = MATCHING_RULES[0]
        # print filter_key

        # filter_sql = "select * from console_apps where downloaded=0 and description like '%" + matching_key + "%'"
        filter_sql = "select * from console_apps where downloaded=0"
        # print filter_sql
        #"type LIKE '%" + filter_key + "%' OR" "description LIKE '%" + filter_key + "%'"
        ...
        #     for r in row:    
        #        print r
        undownloaded_urls = [r[4] for r in records]
    except sqlite3.OperationalError:
